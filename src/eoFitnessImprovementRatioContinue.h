...
#ifndef _eoFitnessImprovementRatioContinue_h
#define _eoFitnessImprovementRatioContinue_h

#include <eoContinue.h>
#include <utils/eoLogger.h>

/**
    A continuator:  stops whenever a given number of generations takes place without a minimum fitness improvement ratio

    @ingroup Continuators
*/
template< class EOT>
class eoFitnessImprovementRatioContinue: public eoContinue<EOT>
{
public:
  typedef typename EOT::Fitness Fitness;

  /// Ctor for setting a
  eoFitnessImprovementRatioContinue(unsigned long _minGens, unsigned long _steadyGens, unsigned long _maxGens, Fitness _minFitnessRatio)
    : repMinGenerations( _minGens ), repSteadyGenerations( _steadyGens),
      repMaxGenerations( _maxGens ), repMinFitnessRatio( _minFitnessRatio),
...
*/
  ...
  virtual bool operator() ( const eoPop<EOT>& _vEO ) {
    ...
    if (thisGeneration > repMaxGenerations)
        return false;
    ...
    if (steadyState) {     // already after MinGenenerations
      ...
      if (((bestCurrentFitness - bestSoFar) / bestSoFar) > repMinFitnessRatio)  {
...
private:
  ...
  unsigned long repMaxGenerations;
